...
if __name__ == "__main__":
    ...
    x1eq = x1
    w = numpy.array([[0,0.1,0.9], [0.1,0.0,0.0], [0.9, 0.0, 0.0]])
    n = x1.size
    ...
    print fx1lin.shape, calc_fx1_2d_taylor(x1, -1.5, z, yc, Iext1, slope, a=1.0, b=-2, tau1=1.0, x1_neg=True, order=2,
                                           shape=(n, ))
    for ii in range(3):
        print fx1lin[ii].expand(sx1[ii]).collect(sx1[ii])
        print sfx1lin[1][ii].expand(sx1[ii]).collect(sx1[ii])
    ...
    for ii in range(n):
        print fx1y1_6d_diff_x1[ii].expand(sx1[ii]).collect(sx1[ii])
        print sfx1y1_6d_diff_x1[1][ii].expand(sx1[ii]).collect(sx1[ii])
