...
if __name__ == "__main__":
    ...
    if model == "EpileptorDP2D":
        ...
        model_vars = 2
        dfun = calc_dfun(eq[0].T, eq[1].T, yc, Iext1, x0, K, w, model_vars, x0cr=x0cr, r=r,
        ...
        jac = calc_jac(eq[0].T, eq[1].T, yc, Iext1, x0, K, w, model_vars, x0cr=x0cr, r=r,
        ...
        if model == "EpileptorDPrealistic":
            ...
            eq, slope_eq, Iext2_eq = calc_eq_11d(x0_6d, K, w, yc, Iext1, Iext2, slope,
                                                 ...
                                                 zmode=zmode, pmode=pmode)
            model_vars = 11
            dfun = calc_dfun(eq[0].T, eq[2].T, yc, Iext1, x0_6d, K, w, model_vars, x0cr, r, zmode, pmode,
            ...
            jac = calc_jac(eq[0].T, eq[2].T, yc, Iext1, x0, K, w, model_vars,
            ...
            model_vars = 6
            dfun = calc_dfun(eq[0].T, eq[2].T, yc, Iext1, x0_6d, K, w, model_vars, x0cr, r, zmode,
                             y1=eq[1].T, x2=eq[3].T, y2=eq[4].T, g=eq[5].T,
                             slope=slope, a=1.0, b=3.0, d=5.0, s=6.0, Iext2=Iext2, gamma=0.1, tau1=1.0, tau0=2857.0,
                             tau2=10.0, output_mode="array")

            jac = calc_jac(eq[0].T, eq[2].T, yc, Iext1, r, K, w, model_vars,
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    if model == "2d":
        logger.info("\nfx1z = " + str(calc_fx1z(x1, x0, K, w, yc, Iext1, x0cr, r, a=1.0, b=-2.0, d=5.0, tau1=1.0,
                                                tau0=1.0, model=model, zmode=zmode)))
    else:
        logger.info("\nfx1z = " + str(calc_fx1z(x1, x0_6d, K, w, yc, Iext1, x0cr, r, a=1.0, b=3.0, d=5.0, tau1=1.0,
                                                tau0=1.0, model=model, zmode=zmode)))

    if model == "2d":
        logger.info("\nsymbolic fx1z = " + str(sfx1z[0](x1, x0, K, w, x0cr, r, yc, Iext1, a, b2, tau1, tau0)))
    else:
        logger.info("\nsymbolic fx1z = " + str(sfx1z[0](x1, x0_6d, K, w, yc, Iext1, a, b6, d, tau1, tau0)))
    ...
    if model == "2d":
        logger.info("\nfx1z_diff = " + str(calc_fx1z_diff(x1, K, w, a=1.0, b=-2.0, d=5.0, tau1=1.0, tau0=1.0,
                                                          model=model, zmode=zmode)))
    else:
        logger.info("\nfx1z_diff = " + str(calc_fx1z_diff(x1, K, w, a=1.0, b=3.0, d=5.0, tau1=1.0, tau0=1.0,
                                                          model=model, zmode=zmode)))

    if model == "2d":
        logger.info("\nsymbolic fx1z_diff = " + str(sfx1z_diff[0](x1, K, w, a, b2, tau1, tau0)))
    else:
        logger.info("\nsymbolic fx1z_diff = " + str(sfx1z_diff[0](x1, K, w, a, b6, d, tau1, tau0)))
    ...
    logger.info("\nfx1lin = " + str(calc_fx1_2d_taylor(x1, -1.5, z, yc, Iext1, slope, a=1.0, b=-2, tau1=1.0,
                                                       x1_neg=True, order=2, shape=(n, ))))
    logger.info("\nsymbolic fx1lin = " + str(sfx1lin[0](x1, -1.5 * numpy.ones(x1.shape), z, yc, Iext1, slope, a, b2,
                                                        tau1)))
    ...
    logger.info("\nfx1y1_6d_diff_x1 = " + str(calc_fx1y1_6d_diff_x1(x1, yc, Iext1, a, b2, d, tau1, tau0)))
    logger.info("\nsymbolic fx1y1_6d_diff_x1 = " + str(sfx1y1_6d_diff_x1[0](x1, yc, Iext1, a, b2, d, tau1)))
    ...
    # x[ix0] = x1eq[ix0]
    # x[iE] = x0[iE]
    # jac = eq_x1_hypo_x0_optimize_jac(x, ix0, iE, x1, z, x0[ix0], x0cr, r, yc, Iext1, K, w)
    x = numpy.empty_like(sx1).flatten()
    x[ix0] = sx1[ix0]
    vz = symbol_eqtn_fz(n, zmode=numpy.array("lin"), z_pos=True, model="2d", x0="x0", K="K")[2]
    x[iE] = sx0[iE]
    p = x1.shape
    opt_fun = eq_x1_hypo_x0_optimize_fun(x, ix0, iE, sx1, numpy.array(sz), sx0[ix0], sx0cr, sr, syc, sIext1, sK, sw)
    logger.info("\nopt_fun = " + str(opt_fun))
    opt_jac = eq_x1_hypo_x0_optimize_jac(x, ix0, iE, sx1, numpy.array(sz), sx0[ix0], sx0cr, sr, sy1, sIext1, sK, sw)
    logger.info("\nopt_jac = " + str(opt_jac))
    ...
    logger.info("\nfz_jac_square_taylor = " + str(calc_fz_jac_square_taylor(z, yc, Iext1, K, w, tau1=1.0, tau0=1.0)))
    logger.info("\nsymbolic fz_jac_square_taylor = " +
                str(lfz_jac_square_taylor(zeq, yc, Iext1, K, w, a, b2, tau1, tau0, x1sq)))
