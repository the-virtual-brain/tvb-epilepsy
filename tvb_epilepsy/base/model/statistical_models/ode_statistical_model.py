import numpy as np
from tvb_epilepsy.base.constants.model_inversion_constants import X1EQ_MIN, X1EQ_MAX, SIG_INIT_DEF, \
...
class ODEStatisticalModel(StatisticalModel):
    ...
    def __init__(self, name='vep_ode', n_regions=0, active_regions=[], n_signals=0, n_times=0, dt=1.0,
                 x1eq_min=X1EQ_MIN, x1eq_max=X1EQ_MAX, sig_init=SIG_INIT_DEF, observation_model=OBSERVATION_MODEL_DEF,
                 # observation_expression=OBSERVATION_EXPRESSION_DEF, euler_method="forward",
                 **defaults):
        super(ODEStatisticalModel, self).__init__(name, n_regions, x1eq_min, x1eq_max, **defaults)
