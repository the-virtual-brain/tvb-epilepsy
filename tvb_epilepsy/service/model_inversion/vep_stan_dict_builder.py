...
def build_stan_model_dict_to_interface_ins(statistical_model, signals, model_inversion,
    ...
    """
    active_regions = statistical_model.active_regions - 1
    nonactive_regions = statistical_model.nonactive_regions - 1
    SC = statistical_model.model_config.model_connectivity
    act_reg_ones = np.ones((statistical_model.number_of_nonactive_regions,))
    x0_lo = X0_MIN
    x0_mu = model_inversion.x0[active_regions]
    x0_hi = 0.5 + x0_mu.max()
    if not statistical_model.priors_mode is PriorsModes.INFORMATIVE:
    ...
    return vep_data, x0_mu, x_init_mu, z_init_mu
