...
class TestTimeseries(object):
    ...
    def test_timeseries_2D(self):
        ts_from_2D = Timeseries(self.data_2D, dimension_labels={TimeseriesDimensions.SPACE: ["r1", "r2", "r3"]},
        ...
        assert ts_from_2D.get_end_time() == 0.02
        assert all(ts_from_2D.get_time_line() == numpy.array([0, 0.01, 0.02]))
        ...
        assert ts_r2.dimension_labels[TimeseriesDimensions.SPACE] == ["r2"]
        assert ts_r2.get_subspace_by_labels(["r2"]).dimension_labels[TimeseriesDimensions.SPACE] == ["r2"]
        ...
        assert ts_r2_idx.dimension_labels[TimeseriesDimensions.SPACE] == ["r2"]
        assert ts_r2_idx.get_subspace_by_index([0]).dimension_labels[TimeseriesDimensions.SPACE] == ["r2"]
        ...
        assert ts_time_window.dimension_labels[TimeseriesDimensions.SPACE] == ["r1", "r2", "r3"]
        ...
        assert ts_time_window_units.dimension_labels[TimeseriesDimensions.SPACE] == ["r1", "r2", "r3"]
    ...
    def test_timeseries_3D(self):
        ts_3D = Timeseries(self.data_3D,
                           dimension_labels={TimeseriesDimensions.SPACE: [], TimeseriesDimensions.STATE_VARIABLES: []},
                           time_start=self.time_start, time_step=self.time_step, time_unit=self.time_unit)
